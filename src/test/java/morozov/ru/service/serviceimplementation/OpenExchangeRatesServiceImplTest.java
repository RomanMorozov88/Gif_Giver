...
public class OpenExchangeRatesServiceImplTest {
    ...
    public void init() {
        ...
        prevRates.setTimestamp(time);
        prevRates.setBase("T_BASE");
        ...
        prevRates.setRates(prevRatesMap);
    ...
    public void whenGotUnExistCharCode() {
        ...
        Mockito.when(openExchangeRatesClient.getHistoricalRates(anyString(), anyString()))
                .thenReturn(prevRates);
        int result = exchangeRatesService.getKeyForTag("UnExist");
        assertEquals(-101, result);
    }
